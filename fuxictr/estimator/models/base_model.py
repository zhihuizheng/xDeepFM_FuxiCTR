...
# from ..tf_utils import get_optimizer, get_loss_fn, get_regularizer
...
class BaseModel(object):
    ...
    def on_batch_end(self, sess, batch, logs={}):
        ...
        if (batch + 1) % self._every_x_batches == 0 or (batch + 1) % self._batches_per_epoch == 0:
            ...
            val_logs = self.evaluate_generator(sess, self.valid_gen)
            # self.checkpoint_and_earlystop(epoch, val_logs)
            logging.info("--- {}/{} batches finished ---".format(batch + 1, self._batches_per_epoch))

    def fit_generator(self, sess, data_generator, epochs=1, validation_data=None,
        ...
        #     epoch_loss = self.train_one_epoch(sess, data_generator, epoch)
        #     # TODO: 放在其他位置？
        #     self.saver.save(sess, '{}/model.ckpt'.format(self.model_dir), global_step=self.global_step)
        ...
        def train_input_fn(data_generator):
            batch_iterator = data_generator
            for batch_index, batch_data in enumerate(batch_iterator):
                yield batch_data[0], batch_data[1]

        self.model.train(input_fn=lambda: train_input_fn(data_generator))

        logging.info("Training finished.")

    # def train_one_epoch(self, sess, data_generator, epoch):
    #     epoch_loss = 0
    #     batch_iterator = data_generator
    #     if self._verbose > 0:
    ...
    #             feed_dict={self.x: batch_data[0],
    #                        self.y: batch_data[1],
    #                        self.is_train: True})
    #         epoch_loss += loss
    #         self.on_batch_end(sess, batch_index)
    #         # if self._stop_training:
    #         #     break
    #     return epoch_loss / self._batches_per_epoch

    def evaluate_generator(self, sess, data_generator):
        y_pred = []
        y_true = []
        if self._verbose > 0:
            from tqdm import tqdm
        ...
        for batch_data in data_generator:
            yp, = sess.run([self.y_pred], feed_dict={self.x: batch_data[0]})
            y_pred.extend(yp)
            y_true.extend(batch_data[1])
