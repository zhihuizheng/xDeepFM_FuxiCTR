...
class EmbeddingDictLayer(object):
    def __init__(self, 
        ...
        self.embedding_layer = OrderedDict() #nn.ModuleDict()
        ...
        for feature, feature_spec in self._feature_map.feature_specs.items():
            if self.is_required(feature):
                if (not use_pretrain) and embedding_dim == 1:
                    ...
                    #     self.embedding_hooks[feature] = tf.layers.dense(feat_emb_dim, embedding_dim, use_bias=False)
                ...
                if feature_spec["type"] == "numeric":
                    #self.embedding_layer[feature] = tf.layers.dense(1, feat_emb_dim, use_bias=False)
                    self.embedding_layer[feature] = tf.get_variable(feature, shape=[embedding_dim], trainable=True)
                elif feature_spec["type"] == "categorical":
                    # padding_idx = feature_spec.get("padding_idx", None)
                    # embedding_matrix = tf.nn.embedding_lookup(feature_spec["vocab_size"],
                    #                                           feat_emb_dim)#,
                                                              # padding_idx=padding_idx)
                    embedding_matrix = tf.get_variable(feature,
                                                       shape=[feature_spec["vocab_size"], embedding_dim],
        ...
        for feature, feature_spec in self._feature_map.feature_specs.items():
            if feature in self.embedding_layer:
                if feature_spec["type"] == "numeric":
                    ...
                    embedding_vec = tf.multiply(inp, self.embedding_layer[feature])
                elif feature_spec["type"] == "categorical":
                    inp = tf.cast(X[:, feature_spec["index"]], tf.int32)
                    embedding_vec = tf.nn.embedding_lookup(params=self.embedding_layer[feature], ids=inp)
                # elif feature_spec["type"] == "sequence":
